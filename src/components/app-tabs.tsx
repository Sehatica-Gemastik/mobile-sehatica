...
export function getTabBarHeight(bottomInset: number) {
  return TAB_CONTENT_HEIGHT + Math.max(bottomInset, 10);
}
...
export function BottomTabBar() {
  ...
  return (
    <View style={[styles.barLayer, { paddingBottom: Math.max(insets.bottom, 10) }]}>
      <View style={[styles.bar, Shadows.sm, { backgroundColor: colors.tabBar }]}>
        {TABS.map((tab) => {
